...
from true_love_ai.llm import LLMService, name as llm_name
...
class ChatMsgHandler:
    def __init__(self):
        ...
        self.chatbot = None
        # 如果是 llm 服务
        if config.ENABLE_BOT == llm_name:
            self.chatbot = LLMService()
...
if __name__ == "__main__":
    print(
        ChatMsgHandler().get_analyze("帮我换一种分割", ChatMsgHandler().get_img("生成一张图片", '', "3", '')['img'],
                                     "3", ""))
