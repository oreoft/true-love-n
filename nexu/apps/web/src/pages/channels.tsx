...
export function ChannelsPage() {
  ...
  const currentChannels = channels.filter((ch) => ch.channelType === platform);
  const isConfigured = currentChannels.length > 0;
    ...
    <div className="px-4 py-4 sm:px-6 sm:py-6 md:p-8 mx-auto max-w-4xl">
      ...
      {showGuide ? (
        ...
        <div className="space-y-6">
          ...
          {platform === "wechat" && (
            <div className="flex justify-center mt-4 border-t border-border pt-6">
              <button
                type="button"
                onClick={() => setForceGuide(true)}
                disabled={quotaLimited}
                className="flex items-center gap-2 px-4 py-2 text-[13px] font-medium text-text-secondary bg-surface-1 border border-border rounded-lg hover:border-border-hover hover:bg-surface-2 transition-all disabled:opacity-60"
              >
                <div className="flex items-center justify-center w-5 h-5 rounded bg-accent/10 text-accent font-bold text-[16px] leading-none pb-[2px]">
                  +
                </div>
                {t("wechatSetup.addAnother", "Add another WeChat account")}
              </button>
            </div>
          )}
        </div>
