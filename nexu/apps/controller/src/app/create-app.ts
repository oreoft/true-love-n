...
export function createApp(container: ControllerContainer) {
  ...
  // In production, serve the built web app (SPA) — no nginx needed
  if (container.env.nodeEnv === "production") {
    const __dirname = path.dirname(fileURLToPath(import.meta.url));
    // Resolve web/dist relative to the controller dist output
    const webDistPath = path.resolve(__dirname, "..", "..", "..", "web", "dist");
    const relativePath = path.relative(process.cwd(), webDistPath) + "/";

    // Serve static assets
    app.use("*", serveStatic({ root: relativePath }));
    // SPA fallback: return index.html for unmatched routes so client-side routing works
    app.use("*", serveStatic({ path: path.join(relativePath, "index.html") }));
  }
